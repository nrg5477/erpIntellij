...
public class BoardController {
	...
	public ModelAndView list(@RequestParam("category") String category) {
		...
		//카테고리 선택 정보를 넘겨주기 위해서 뷰에서 요청이 왔다가 돌아갈때 http통신이라서 정보를 가지고 가지않음.
	...
	//스프링 MVC에서 클라이언트가 입력한 데이터를 객체로 만들어서 컨트롤러에 전송.
	//                                  --BoardDTO : 업로드하는 파일을 받을 수 있도록 컬럼을 추가
	...
	public ResponseEntity<UrlResource> downloadFile(@PathVariable String id, @PathVariable String board_no, @PathVariable String boardFileno, HttpSession session) throws MalformedURLException, FileNotFoundException {
		...
		String mycontentttype="attachment; filename=\""+encodedFilename+"\"";
		//응답메시지만들기
//		BodyBuilder builder = ResponseEntity.ok();//=>response가 정상처리 되었는다는것을 인식시키기 위해서 셋팅
//		buider.header
//		ResponseEntity<UrlResource> response = builder.body(resource);
//		return response;
		return ResponseEntity.ok().header(HttpHeaders.CONTENT_DISPOSITION, mycontentttype).body(resource);
	}
